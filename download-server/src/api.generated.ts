...
export const createApiRequest = (rs: IRequestService) => ({
    actorIdDelete: (
        id: string,
    ): CustomPromise<CustomAxiosResponse<Array<IActorResponse>, TActorIdDeleteError>, IBEError<TActorIdDeleteError>> =>
        rs.delete(formatUrl(API_SERVER_URL + `api/actor/${id}`)),

    actorIdGet: (
        id: string,
    ): CustomPromise<CustomAxiosResponse<IActorResponse, TActorIdGetError>, IBEError<TActorIdGetError>> =>
        rs.get(formatUrl(API_SERVER_URL + `api/actor/${id}`)),

    actorIdPut: (
        id: string,
        body: IPutActorBody,
    ): CustomPromise<CustomAxiosResponse<IActorResponse, TActorIdPutError>, IBEError<TActorIdPutError>> =>
        rs.put(formatUrl(API_SERVER_URL + `api/actor/${id}`), body),

    actorGet: (
        query: { actor_name?: string; movie_id?: string; imdb_id?: string } | undefined,
    ): CustomPromise<CustomAxiosResponse<Array<IActorResponse>, TActorGetError>, IBEError<TActorGetError>> =>
        rs.get(formatUrl(API_SERVER_URL + `api/actor/`, query)),

    actorPost: (
        body: IPostActorBody,
    ): CustomPromise<CustomAxiosResponse<Array<IActorResponse>, TActorPostError>, IBEError<TActorPostError>> =>
