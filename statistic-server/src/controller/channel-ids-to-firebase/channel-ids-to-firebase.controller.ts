...
import { IChannelIdsToFirebaseBody } from '@common/model/statistic-server.model';
...
app.get(API_URL.api.channelIdsToFirebase.toString(), async (req: IRequest, res: IResponse) => {
    ...
    const [data, error] = await allServices.statistic.channelIdsToFirebaseAsync(req.body, logger);
    if (error) {
