...
interface IExistRequest extends IExpressRequest {
    body: IExistBody;
}

interface IExistResponse extends IExpressResponse<string[], void> {}

app.post(API_URL.api.channel.exist.toString(), async (req: IExistRequest, res: IExistResponse) => {
    const logger = createLogger();
    const [data, error] = await allServices.channel.existAsync(req.body.ids, logger );
    if (error) {
...
app.get(API_URL.api.channel.toString(), async (req: IListRequest, res: IListResponse) => {
    const logger = createLogger();
    const [data, error] = await allServices.channel.getChannelListAllAsync(req.query, logger);
    if (error) {
